...
export const Resizer = ({ propKey, children, ...props }: any) => {
  ...
  const getUpdatedDimensions = (width: number, height: number) => {
    ...
    const currentWidth = parseInt(editingDimensions.current.width || '0'),
      currentHeight = parseInt(editingDimensions.current.height || '0');
  ...
  return (
    <Resizable
      ...
      ref={(ref) => {
        if (ref) {
          const resizableElement = ref.resizable;
          if (resizableElement) {
            connect(resizableElement);
          }
        }
      }}
